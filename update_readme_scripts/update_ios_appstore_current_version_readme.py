...
def parse_latest_xml(file_path):
    ...
    for package in root.findall("package"):
        package_info = {
            ...
            "app_store_url": package.find("app_store_url").text.strip(),  # NEW
        }
    ...
    for package in packages:
        ...
        if package['icon_image'] != 'NA':
            if package.get('app_store_url') and package['app_store_url'] != 'NA':
                icon_html = (
                    f"<a href='{package['app_store_url']}' target='_blank' rel='noopener'>"
                    f"<img src='{package['icon_image']}' alt='{package['application_name']} icon' width='60' style='border-radius: 12px;' />"
                    f"</a>"
                )
            else:
                icon_html = (
...
if __name__ == "__main__":
    ...
    readme_file_path = "docs/ios_appstore/ios_appstore_current_version.md"
